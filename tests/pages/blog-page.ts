...
export class BlogPage {
    ...
    async searchBlog(data: string) {
        ...
        await this.page.screenshot({path:  `./tests/screenshot/blog-page-${data}.png`});

    }

    async checkTitleResult(data: string) {
        await expect(this.titleSearch).toBeVisible();
        await expect(this.titleSearch).toContainText(messages.resultPage.title);
        await this.page.screenshot({path: `./tests/screenshot/result-search-page-${data}.png`});
    }
    ...
    async checkArticleResult(data: string) {
       ...
       await this.page.screenshot({path: `./tests/screenshot/article-search-page-${data}.png`});
    }
    ...
    async checkInvalidSearch(invalid: string) {
        ...
        await this.page.screenshot({path: `./tests/screenshot/invalid-search-page.png`});
